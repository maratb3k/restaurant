...
public class ReportGeneratorUtil {
    ...
    public void generateWeeklyReports() {
        try {
            ...
            for (String locationId : locationIds) {
                Reports staffReport = calculateStaffPerformance(locationId, periodStart, periodEnd, prevStart, prevEnd);
                sendToKafka(staffReport);

                Reports locationReport = calculateLocationPerformance(locationId, periodStart, periodEnd, prevStart, prevEnd);
                sendToKafka(locationReport);
            }
        } catch (Exception e) {
    ...
    private Reports calculateStaffPerformance(String locationId, LocalDate periodStart, LocalDate periodEnd, LocalDate prevStart, LocalDate prevEnd) {
        ...
        for (Waiter waiter : waiters) {
            int workingHours = waiter.getTimeSlots().entrySet().stream()
                    .filter(e -> {
                        String dateTimeStr = e.getKey().split("_")[0];
            ...
            List<Feedback> feedbacks = feedbackRepository.findAllByIdIn(
            ...
            List<Feedback> prevFeedbacks = feedbackRepository.findAllByIdIn(
    ...
    private Reports calculateLocationPerformance(String locationId, LocalDate periodStart, LocalDate periodEnd, LocalDate prevStart, LocalDate prevEnd) {
        ...
        List<Feedback> feedbacks = feedbackRepository.findAllByIdIn(
                reservations.stream().map(Reservation::getFeedbackId).filter(Objects::nonNull).collect(Collectors.toList()));
        double avgCuisineFeedback = feedbacks.stream().mapToInt(Feedback::getCuisineRating).average().orElse(0);
        int minCuisineFeedback = feedbacks.stream().mapToInt(Feedback::getCuisineRating).min().orElse(0);
    ...
    private void sendToKafka(Reports metrics) {
        try {
            String messageBody = objectMapper.writeValueAsString(metrics);
            kafkaTemplate.send(reportsTopic, metrics.getLocationId(), messageBody);

            log.info("Sent {} report for location {} to Kafka topic {}.",
                    metrics.getReportType(), metrics.getLocationId(), reportsTopic);
        } catch (Exception e) {
            log.error("Failed to send Kafka message for {} report, location {}",
